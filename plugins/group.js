...
bot(
  ...
  async (message, match) => {
    ...
    if (!isImAdmin) return await message.send(`_You idiot.I'm not admin in this group._`)
...
bot(
  ...
  async (message, match) => {
    await message.send('> dont add those not in contacts, chances of a ban is high.Just Kiddig.Well im not')
    const participants = await message.groupMetadata(message.jid)
    const isImAdmin = await isAdmin(participants, message.client.user.jid)
    if (!isImAdmin) return await message.send(`_You Fool.I'm not admin here.Promote me then i will be able to add contacts._`)
...
bot(
  ...
  async (message, match) => {
    const participants = await message.groupMetadata(message.jid)
    const isImAdmin = await isAdmin(participants, message.client.user.jid)
    if (!isImAdmin) return await message.send(`_Give me admin role first._`)
...
bot(
  ...
  async (message, match) => {
    const participants = await message.groupMetadata(message.jid)
    const isImAdmin = await isAdmin(participants, message.client.user.jid)
    if (!isImAdmin) return await message.send(`_Can't believe this guy, im not admin here._`)
    const user = message.mention[0] || message.reply_message.jid
    if (!user) return await message.send(`_Give me a user._`)
    const isUserAdmin = await isAdmin(participants, user)
...
bot(
  ...
  async (message, match) => {
    const participants = await message.groupMetadata(message.jid)
    const isImAdmin = await isAdmin(participants, message.client.user.jid)
    if (!isImAdmin) return await message.send(`_Hey dude, promote me first._`)
...
bot(
  ...
  async (message, match) => {
    const participants = await message.groupMetadata(message.jid)
    const isImAdmin = await isAdmin(participants, message.client.user.jid)
    if (!isImAdmin) return await message.send(`_Let them have fun.just kidding promote me first._`)
...
bot(
  ...
  async (message, match) => {
    const participants = await message.groupMetadata(message.jid)
    const isImAdmin = await isAdmin(participants, message.client.user.jid)
    if (!isImAdmin) return await message.send(`_Does it look like im admin._`)
    return await message.GroupSettingsChange(message.jid, false)
  }
)
...
bot(
  ...
  async (message, match) => {
    ...
    return await message.send(`_i'm in. Promote me to use my group cmds_`)
  }
)
